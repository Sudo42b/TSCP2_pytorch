...
class AttentionBlock(nn.Module):
    def __init__(self, embed_dim:int= None, 
                    ...
                    value_dim:int= None, 
                    dropout:int = 0.1):
        """An attention mechanism Scale Dot Product Attention et al (2017)
            Reference: https://github.com/flrngel/TCN-with-attention
            Time Series Batch-Size, Uni(Multi)-Variate TS, Windows
            
            The input of the AttentionBlock is `BxDxT` where 
            `B` is the input 'Batch size', -> 0th
            `D` is the `dimensions of each Feature`. 1th
            `T` is the length of the `Time Sequence` 2th
            
                'Attention = softmax( (Q*K.T)/sqrt.dK )*V'
            
            The output of the AttentionLayer is `BxTx(D+"V")` where `V` is the size of the
            attention values.

        Args:
            embed_dim (int, optional): the number of dimensions (or channels) of each element in
                                    the length of the input and output sequences.
            key_dim (int, optional): the size of the attention keys. Defaults to None.
            value_dim (int, optional): the size of the attention values. Defaults to None.
            dropout (int, optional): drop-out rate. Defaults to 0.1.
        """
        super(AttentionBlock, self).__init__()
        
        assert embed_dim is not None, "Provide either embed_dim"
        
        value_dim = value_dim or embed_dim
        self.key_dim = value_dim or embed_dim
        
        self.key_W = nn.Linear(embed_dim, key_dim)
        self.query_W = nn.Linear(embed_dim, key_dim)
        self.value_W = nn.Linear(embed_dim, value_dim)
        
        self.d_k = 1/math.sqrt(key_dim)
    ...
    def forward(self, query):
        B, L, E = query.size() #B=batch_size, L=Length, E= Feature Len
        
        K = self.key_W(query)
        Q = self.query_W(query)
        V = self.value_W(query)
        
        #Q shape is (B, L, E), K shape is (B, E, L)
        #BMM to Q*K = (B, L, L)
        Q = Q.permute(0, 2, 1)
        scores = torch.einsum("bij,bjk->bik", Q, K) * self.d_k
        
        attn_mask = TriangularCausalMask(B, E, device=V.device)
